...
class PriceAddition(Addition):
    ...
    def apply(self):
        ...
        providerName = self.cmbProvider.get()
        providerCode = self.db.getCode('provider_list', providerName)
        ...
        if (repeatFlag is True and
            ...
            providerName = self.db.getName('provider_list', providerCode)
            ...
            messagebox.showerror(
                'Ошибка',
                'Поставщик не указан или указанный поставщик не найден.',
                parent=self.toplevel)

    def clear(self):
        [w.set('') for w in (self.cmbManufacturer, self.cmbProvider)]
    ...
    def fillAndShow(self):
        if self.tv.selection()[0]:
            if len(self.tv.parent(self.tv.selection()[0])):
                ...
                self.entPrice.insert(0, values[2])
                self.entDate.insert(0, values[3])
                self.show()

    def apply(self):
        slcTag = self.tv.selection()[0]
        price = self.trn.getFltOrZero(self.entPrice.get())
        self.tv.set(slcTag, 2, '{:.2f}'.format(price))
        self.tv.set(slcTag, 3, self.entDate.get())
