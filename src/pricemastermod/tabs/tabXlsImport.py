...
class ImportXlsTab(XlsTab):
    def __init__(self, basis):
        ...
        [w.grid(row=i, **self.grd.cl1_sp2_pdSt_stWE) for i, w in enumerate(
            (self.cmbItmCol, self.cmbMnfCol, self.cmbPrcCol, self.cmbExpDtCol),
            start=2)]
        lblPriceDate.grid(row=6, **self.grd.cl0_pdSt_stW)
        self.entPriceDate.grid(row=6, **self.grd.cl1_sp2_pdSt_stWE)
        self.btnImportData.grid(row=7, **self.grd.cl0_sp3_pdSt_stWE)
    ...
    def importData(self):
        self.getParameters()
        prvName = self.db.data['provider']['name']
                ...
                if prvName != '' and self.itmColID > -1 and self.mnfColID > -1:
                    ...
                    if repeatFlag is True:
                        self.db.data['price_list'].append({
                            ...
                            'price': price,
                            'date': expiryDate})
                self.progress('Импорт записей', index, strLen)
            self.progress('Импорт завершён', index, strLen)
            self.db.saveData()
